...
import static com.cicada.player.app.view.subtitle.LocationStyle.Location_CenterH;
import static com.cicada.player.app.view.subtitle.LocationStyle.Location_Top;
...
import com.cicada.player.app.util.HttpClientHelper;
...
import com.cicada.player.app.view.GestureDialogManager;
 ...
 */
public class CicadaVodPlayerView extends RelativeLayout implements MediaController.MediaPlayerControl {
     ...
     */
    private boolean mNeedToRetry = true;
...
//    private ThumbnailHelper mThumbnailHelper;
    ...
    CicadaPlayer.OnVideoSizeChangedListener mSizeChangedListener = new CicadaPlayer.OnVideoSizeChangedListener() {
        @Override
...
/*        if (mTipsView.isErrorShow()) {
    ...
    private void networkRetry() {
        Log.d(TAG, "networkRetry()  " + mRetryTime + " " + mNeedToRetry);
        if (mRetryTime > 0) {
            if (mNeedToRetry) {
...
//                    }
    ...
    private void initCicadaPlayer() {
        ...
        Logger.getInstance(getContext()).setLogLevel(Logger.LogLevel.AF_LOG_LEVEL_WARNING);
        ...
        mCicadaVodPlayer.enableHardwareDecoder(true);
...
//                    setCoverUri(mAliyunMediaInfo.getCoverUrl());
                ...
                //初始化屏幕亮度
                mMediaController.setScreenBrightness(SysUtils.BrightnessUtil.getUserBrightness(mContext));
...
//        }
    ...
    public int getDuration() {
        if (mCicadaVodPlayer != null) {         // && isPlayingOrPaused()
            Log.d(TAG, "getDuration()  " + mCicadaVodPlayer.getDuration());
            return (int) mCicadaVodPlayer.getDuration();
        }

        Log.d(TAG, "getDuration()   -1");
        return -1; // 0
    }
...
//            mCoverView.setVisibility(GONE);
    ...
    public void start() {
        ...
        if (currentPlayState == CicadaPlayer.prepared || isPlayingOrPaused()) {
            mCicadaVodPlayer.start();
            updateViewState(ControlView.PlayState.Playing);
        }

        if (mVideoPlayerActivity != null)
            mVideoPlayerActivity.doStart();
    }
...
*/
    ...
    public CicadaPlayer.ScaleMode getScaleMode() {
        if (mCicadaVodPlayer != null) {
            return mCicadaVodPlayer.getScaleMode();
        }
    ...
    public float getSpeedMode() {
        return mCicadaVodPlayer.getSpeed();
    }
    ...
    public void setSpeedMode(float speed) {
        if (mCicadaVodPlayer != null) {
            mCicadaVodPlayer.setSpeed(speed);
            Toast.makeText(getContext(), "播放速度 " + speed + " 倍", Toast.LENGTH_SHORT).show();
        }
    }
...
//    }
    ...
    public void setActivity(CicadaVideoPlayer activity) {
        mVideoPlayerActivity = activity;
    }
    ...
    public void finish() {
        stop();
        if (mVideoPlayerActivity != null) {
            mVideoPlayerActivity.finish();
        }
    }

    @Override
    public int getVolumeMode() {
        return mVideoPlayerActivity.getVolumeMode();
    }
    ...
    public int getDefaultWidth() {
        ...
        return mCicadaVodPlayer.getVideoWidth();
    }
    ...
    public int getDefaultHeight() {
        ...
        return mCicadaVodPlayer.getVideoHeight();
    }

    @Override
    public void setScreenSize(int width, int height) {
        Log.d(TAG, "setScreenSize()  width = [" + width + "], height = [" + height + "]");
    ...
    public void selectPlayMode() {
        mVideoPlayerActivity.selectPlayMode();
    }
    ...
    @Override
    public boolean prev() {
        return mVideoPlayerActivity.doPrevious();
    }

    @Override
    public boolean next() {
        return mVideoPlayerActivity.doNext();
    }

    @Override
    public int getCurrentPosition() {
        if (isInPlaybackState()) {
            return mControlView.getVideoPosition();
        }
        return 0;
    }
    ...
    private void attachMediaController() {
        if (mCicadaVodPlayer != null && mMediaController != null) {
