...
extern unsigned short g_port_seted;
extern unsigned short g_event_port;


struct httpd_callbacks_s {
	void* opaque;
	void* (*conn_init)(void *opaque, unsigned char *local, int locallen, unsigned char *remote, int remotelen);
	void  (*conn_request)(void *ptr, http_request_t *request, http_response_t **response, int index);
...
httpd_t *httpd_init(httpd_callbacks_t *callbacks, int max_connections, int id);
...
int httpd_get_socket(httpd_t *httpd);
