...
http_response_t *
http_response_init_ex(const char *protocol, const char *action, const char *message)
{
	http_response_t *response;
	...
	http_response_add_data(response, protocol, strlen(protocol));
	http_response_add_data(response, " ", 1);
	http_response_add_data(response, action, strlen(action));
	http_response_add_data(response, " ", 1);
	http_response_add_data(response, message, strlen(message));
	http_response_add_data(response, "\r\n", 2);
...
int http_response_get_flag(http_response_t *response)
{
	return response->flag;
}

void http_response_set_flag(http_response_t *response, int flag)
{
	response->flag = flag;
	return;
}
