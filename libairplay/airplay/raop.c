...
// multi device
struct AesKeyContext{
	AES_KEY g_aes_ctr_handle ;
	unsigned char g_ctr_key[16] ;
	unsigned char g_ctr_iv[16]  ;
	unsigned char g_ctr_ec[16]  ;
	unsigned int  g_ctr_num ;
};
unsigned char message3[164] = {0};

void* g_aes_key_ctx = NULL; //每个device的设备不一样

void g_aes_crt_setkey(unsigned char* key, unsigned char* iv) {
	g_aes_key_ctx = calloc(1, sizeof(struct AesKeyContext));
	struct AesKeyContext* ctx = g_aes_key_ctx;

	memcpy(ctx->g_ctr_key, key, 16);
	memcpy(ctx->g_ctr_iv, iv, 16);
	AES_set_encrypt_key(ctx->g_ctr_key, 128, &ctx->g_aes_ctr_handle);
}

void g_aes_ctr_encrypt(unsigned char* data, int size, void *aes_ctx) {
	struct AesKeyContext* ctx = aes_ctx;
	new_AES_ctr128_encrypt(data, data, size, &ctx->g_aes_ctr_handle, ctx->g_ctr_iv, ctx->g_ctr_ec, &ctx->g_ctr_num);
}
 
void* airdata_thread_proc(void* lpParameter) {
	airdata_t * airplay;
	signed int v2; // edi@1
	int sock; // esi@1
	int nfds; // eax@6
	...
	mutex_handle_t l_mutex; // [sp-4h] [bp-81ACh]@2
	struct timeval timeout; // [sp+Ch] [bp-819Ch]@5
	socklen_t addrlen; // [sp+14h] [bp-8194h]@12
	int sock_; // [sp+18h] [bp-8190h]@12
	...
	sock = -1;
	v16 = 0;
	if(DBG) ALOGI("========Enter TCP RAOP thread========");
	while (1) {
		...
		if (v2) {
			...
			if (sock == -1) {
				...
				FD_SET(airplay->m_socket,&readfds);
				// readfds.fds_bits[0] = airplay->m_socket;
				nfds = airplay->m_socket + 1;
			} else {
				// readfds.fd_array[0] = sock;
				// nfds = sock + 1;
				FD_SET(sock,&readfds);
				// readfds.fds_bits[0] = sock;
				nfds = sock + 1;
			}
			...
			if (ret) {
				...
				if (sock == -1) {
					if (FD_ISSET(airplay->m_socket, &readfds)) {
						...
						v7 = (uint32_t*)(__errno)();
						v8 = strerror(*v6);
						if(DBG) ALOGI("Error in accept %d %s", *v7, v8);
						sock = sock_;
						v2 = 1;
						v16 = 1;
					}
...
LABEL_14:
					if (FD_ISSET(sock, &readfds)) {
						...
						if(DBG) ALOGW("===============lijun=======packetlen: %d=================", recv_len);
						if (!recv_len) {
							if(DBG) ALOGI("TCP socket closed");
							goto LABEL_21;
						}
						bo_error = recv_len == -1;
						v2 = v16;
						if (bo_error) {
							if(DBG) ALOGI("Error in recv");
							goto LABEL_21;
						}
					}
				}
...
LABEL_21:
	if (sock != -1)
		closesocket(sock);
	if(DBG) ALOGI("Exiting TCP RAOP thread");
	return 0;
}

// uint32_t __stdcall airdata_thread_proc(LPVOID lpParameter) {
// 	airdata_t * airplay;
// 	signed int v2; // edi@1
// 	int sock; // esi@1
// 	int nfds; // eax@6
...
void airdata_start(airdata_t *airdata, unsigned short *port) {
	...
	if (!airdata->m_started && airdata->m_terminated) {
		...
		if (airdata_init_socket(airdata, use_ipv6) != 0) {
			if(DBG) ALOGI("Initializing sockets failed");
...
void airdata_stop(airdata_t *airdata) {
	// void *mutex; // ST04_4@3  cz
	int v2; // eax@3
	if(DBG) ALOGI("airdata_stop %s:%d  \n",__func__, __LINE__);
	MUTEX_LOCK(airdata->m_mutex);
	if (airdata->m_started && !airdata->m_terminated) {
		if(DBG) ALOGI("airdata_stop %s:%d  \n",__func__, __LINE__);
		// mutex = airdata->m_mutex; // cz
		airdata->m_started = 0;
		MUTEX_UNLOCK(airdata->m_mutex);
...
void airdata_free(airdata_t *a1) {
	if (a1) {
		airdata_stop(a1);
		// CloseHandle(a1->m_mutex);  cz
		// close(a1->m_mutex);
		MUTEX_DESTROY(a1->m_mutex);
		free(a1);
	}
}

airdata_t * raop_rtp_parse_remote(const char *remote) {
	char *original;
	char *current;
	char *tmpstr;
	int family;
	...
	//assert(raop_rtp);
	// current = original = _strdup(remote); cz
...
conn_init(void *opaque, unsigned char *local, int locallen, unsigned char *remote, int remotelen)
{
	...
	conn->controlPort = 0;
	conn->timingPort = 0;
	conn->dataPort = 53291;   //7100 -> 53291
	conn->eventPort = 55556;

	conn->airdata_1 = 0;
	conn->airdata_2 = 0;
	if(DBG) ALOGI("raop  sizeof(conn->nonce)::%d",sizeof(conn->nonce));
	if(DBG) ALOGI("raop  sizeof(raop_conn_t)::%d",sizeof(raop_conn_t));
	if(DBG) ALOGI("raop  conn->nonce::%s", bin2hex(conn->nonce,sizeof(conn->nonce)));
...
http_response_t * request_handle_feedback(raop_conn_t *conn, http_request_t *request, http_response_t *response, char **pResponseData, int *pResponseDataLen) {
	conn->raop->callbacks.mirroring_live(conn->raop->callbacks.cls);
...
http_response_t * request_handle_teardown(raop_conn_t *conn, http_request_t *request, http_response_t *response, char **pResponseData, int *pResponseDataLen) {
	...
	ALOGI("cz0 %s:%d %d ", __func__, __LINE__, (uint32_t)n_type);
    if (n_type == 96) {
        if (conn->raop_rtp) {
            /* Destroy our RTP session */
			ALOGI("cz %s:%d", __func__, __LINE__);
		...
		ALOGI("cz %s:%d", __func__, __LINE__);
        conn->raop->callbacks.mirroring_stop(conn->raop->callbacks.cls, conn->remote );
        //g_set_codec = 0;
    } else if (n_type == 0) {
		ALOGI("cz %s:%d", __func__, __LINE__);
        conn->raop->callbacks.AirPlayPlayback_Stop(conn->raop->callbacks.cls); // 允许用户在手机停止视频播放
    }
...
http_response_t * request_handle_flush(raop_conn_t *conn, http_request_t *request, http_response_t *response, char **pResponseData, int *pResponseDataLen) {
	const char *rtpinfo;
	int next_seq = -1;
	rtpinfo = http_request_get_header(request, "RTP-Info", 1);
	if (rtpinfo) {
		if(DBG) ALOGI("Flush with RTP-Info: %s", rtpinfo);
	...
	if (conn->raop_rtp) {
		...
		if(DBG) ALOGW("RAOP not initialized at FLUSH");
	}
	return response;
}

//SET_PARAMETER
http_response_t * request_handle_setparameter(raop_conn_t *conn, http_request_t *request, http_response_t *response, char **pResponseData, int *pResponseDataLen, int index) {
	const char *content_type;
	const char *data;
	char *contentLengthStr = NULL;
	int datalen;
	int contentLength;
	if(DBG) ALOGD("%s(%d)", __func__, __LINE__);
	http_request_dump_headers(request);
	content_type = http_request_get_iheader(request, "Content-Type", index);
	contentLengthStr = http_request_get_iheader(request, "Content-Length", index);
	if(contentLengthStr) contentLength = atoi(contentLengthStr);
	data = http_request_get_data(request, &datalen);
	datalen = contentLength < datalen ? contentLength : datalen;
	for(; index > 1; index--) {
		contentLengthStr = http_request_get_iheader(request, "Content-Length", index-1);
		if(contentLengthStr) contentLength = atoi(contentLengthStr);
		if(contentLength > 0)
			data += contentLength;
	}
	if (!strcmp(content_type, "text/parameters")) {
		...
		if (data && datastr && conn->raop_rtp) {
			...
			if(DBG) ALOGW("RAOP not initialized at SET_PARAMETER");
		}
		free(datastr);
	} else if (!strcmp(content_type, "image/jpeg") || !strcmp(content_type, "image/png")) {
		if(DBG) ALOGI("Got image data of %d bytes", datalen);
		if (conn->raop_rtp) {
			raop_rtp_set_coverart(conn->raop_rtp, data, datalen);
		} else {
			if(DBG) ALOGW("RAOP not initialized at SET_PARAMETER coverart");
		}
	} else if (!strcmp(content_type, "application/x-dmap-tagged")) {
		if(DBG) ALOGI("Got metadata of %d bytes", datalen);
		if (conn->raop_rtp) {
			raop_rtp_set_metadata(conn->raop_rtp, data, datalen);
		} else {
			if(DBG) ALOGW("RAOP not initialized at SET_PARAMETER metadata");
		}
	}
	//http_response_add_header(response, "Audio-Jack-Status", "connected; type=analog");
	return response;
...
http_response_t * request_handle_streamxml(raop_conn_t *conn, http_request_t *request, http_response_t *response, char **pResponseData, int *pResponseDataLen) {
    ...
    if(DBG) ALOGI("request_handle_streamxml  h %d w %d", conn->raop->height, conn->raop->width);
...
http_response_t * request_handle_info(raop_conn_t *conn, http_request_t *request, http_response_t *response, char **pResponseData, int *pResponseDataLen) {
	...
	sprintf(
		...
		conn->raop->height,
		conn->raop->maxfps);
	if(DBG) ALOGD("info buffer :: %s", buffer);
...
http_response_t * request_handle_setup(raop_conn_t *conn, http_request_t *request, http_response_t *response, char **pResponseData, int *pResponseDataLen) {
	...
	const char* user_agent = http_request_get_header(request, "User-Agent", 1);
	const char* app_pd = http_request_get_iheader(request, "X-Apple-PD", 1);
	...
	transport = http_request_get_header(request, "Transport", 1);
	assert(transport);
	dacp_id = http_request_get_header(request, "DACP-ID", 1);
	active_remote_header = http_request_get_header(request, "Active-Remote", 1);
	if (dacp_id && active_remote_header) {
		if(DBG) ALOGD("DACP-ID: %s", dacp_id);
		if(DBG) ALOGD("Active-Remote: %s", active_remote_header);
		if (conn->raop_rtp) {
    ...
    */
	...
	if (content_type && !strcmp(content_type, "application/x-apple-binary-plist")) { //如果是二进制数据
		...
		if (plist_dict_get_size(p_dict)) {
            ...
            if (p_eiv != NULL) {
				...
				if (p_eiv_data) {
					...
					if(DBG) ALOGD("aesivlen: %d", p_eiv_size);
				}
			}
			//获取aes_key
            ...
            if (p_ekey != NULL) {
				...
				if (p_ekey_data && p_ekey_size == 72) {// ekey 需要通过 DRM 在线解密
					int key_size = 16;
					unsigned char* key_data = fairplay_decrypt(message3, p_ekey_data);
					if(DBG) ALOGD("message3: %s \n", bin2hex(message3,164));
					// unsigned char* key_data = fairplay_query(3, (const unsigned char*)p_ekey_data, p_ekey_size, &key_size);
					if (key_data && key_size == 16) {
						if(DBG) ALOGD("===============phase3=========length=============%d=====\n", 16);
					...
					if (arr_size > 0) {
						...
						do {
							...
							if (n_type == 96) { //音频
								...
								if (n_audioFormat != 0x40000) {
									if (n_audioFormat != 0x400000) {
										if (n_audioFormat != 0x1000000) {
											if(DBG) ALOGD("unsupport audio format");
										} else {
								...
								conn->raop_rtp = raop_rtp_init(&raop->callbacks, buf_remote, buf_rtpmap, buf_ftmp, (const unsigned char*)conn->rsa_cbc_key, (const unsigned char*)conn->rsa_cbc_iv);
								if (conn->raop_rtp) {   //, 1, use_udp
									...
									if(DBG) ALOGD("conn->dataPort1111 %d",conn->dataPort);
								} else {
									if(DBG) ALOGE("RAOP not initialized at SETUP, playing will fail!");
								...
								if (!conn->eventPort) {
									...
									conn->airdata_2 = raop_rtp_parse_remote(buf_remote);
									if (!conn->airdata_2) {
										if(DBG) ALOGE("Error initializing the audio decoder");
										http_response_set_disconnect(response, 1);
									}
									if (conn->airdata_2) {
								...
								if (buf_ftmp)
									free(buf_ftmp);
								if(DBG) ALOGD("Found audio stream\n");	
								...
								if (!conn->eventPort) {
									...
									if (conn->locallen == 4) {
										sprintf(buf_remote, "%s %d.%d.%d.%d", "IN IP4", conn->remote[0], conn->remote[1], conn->remote[2], conn->remote[3]);
										if(DBG) ALOGD("USE IPV4 %s\n", buf_remote);
									} else {
										if(DBG) ALOGD("USE IPV6\n");
									}
									if (conn->airdata_2) {
										airdata_free(conn->airdata_2);
										conn->airdata_2 = 0;
		...
		transport = http_request_get_header(request, "Transport", 1);
		assert(transport);
		if(DBG) ALOGI("Transport: %s", transport);
		...
		if (use_udp) {
			...
			if (original) {
				while ((tmpstr = utils_strsep(&current, ";")) != NULL) {
					...
					if (ret == 1) {
						if(DBG) ALOGD("Found remote control port: %hu", value);
						remote_cport = value;
					}
					ret = sscanf(tmpstr, "timing_port=%hu", &value);
					if (ret == 1) {
						if(DBG) ALOGD("Found remote timing port: %hu", value);
						remote_tport = value;
					}
				}
			}
		...
		if(DBG) ALOGI("Responding with %s", buffer);
...
http_response_t * request_handle_announce(raop_conn_t *conn, http_request_t *request, http_response_t *response, char **pResponseData, int *pResponseDataLen) {
	...
	if (data) {
		...
		if(DBG) ALOGD("connection: %s", remotestr);
		if(DBG) ALOGD("rtpmap: %s", rtpmapstr);
		if(DBG) ALOGD("fmtp: %s", fmtpstr);
		if(DBG) ALOGD("rsaaeskey: %s", aeskeystr);
		if(DBG) ALOGD("aesiv: %s", aesivstr);
		if(DBG) ALOGD("fpaeskey: %s", fpaeskeystr);
		...
		if (fpaeskeystr && !aeskeystr) {
			...
			if (buf && len == 72) {
				...
				if(DBG) ALOGD("base64 decode fail len=%d", len);
			}
		} else {
		...
		if(DBG) ALOGD("aeskeylen: %d", aeskeylen);
		if(DBG) ALOGD("aesivlen: %d", aesivlen);
		if (conn->raop_rtp) {
			/* This should never happen */
			raop_rtp_destroy(conn->raop_rtp);
			conn->raop_rtp = NULL;
		}
		conn->raop_rtp = raop_rtp_init(&raop->callbacks, remotestr, rtpmapstr, fmtpstr, aeskey, aesiv);
		if (!conn->raop_rtp) {
			if(DBG) ALOGE("Error initializing the audio decoder");
			http_response_set_disconnect(response, 1);
		}
...
http_response_t * request_handle_fpsetup(raop_conn_t *conn, http_request_t *request, http_response_t *response, char **pResponseData, int *pResponseDataLen) {
	...
	if(datalen == 16){
		size = 142;
		buf = fairplay_setup(data, datalen);
		if(DBG) ALOGD("1buf: %s \n", bin2hex(buf,size));
	}else{
		size = 32;
		buf = fairplay_setup(data, datalen);
		if(DBG) ALOGD("2buf: %s \n", bin2hex(buf,size));
		memcpy(message3, data, 164);
		if(DBG) ALOGD("message3: %s \n", bin2hex(message3,datalen));
	}
...
http_response_t * request_handle_authorization(raop_conn_t *conn, http_request_t *request, http_response_t *response, int *p_require_auth, char **pResponseData, int *pResponseDataLen) {
	...
	authorization = http_request_get_header(request, "Authorization", 1);
	if (authorization) {
		if(DBG) ALOGD("Our nonce: %s", conn->nonce);
		if(DBG) ALOGD("Authorization: %s", authorization);
	}
	if (!digest_is_valid(realm, raop->password, conn->nonce, method, http_request_get_url(request, 1), authorization)) {
		char *authstr;
		int authstrlen;
		...
		*p_require_auth = 1;
		...
		free(authstr);
		if(DBG) ALOGD("Authentication unsuccessful, sending Unauthorized");
	} else {
		*p_require_auth = 0;
		if(DBG) ALOGD("Authentication successful!");
	}
...
static void conn_request(void *ptr, http_request_t *request, http_response_t **response, int index) {
	...
	uri = http_request_get_url(request, index);
	cseq = http_request_get_header(request, "CSeq", index);
	if (!method) {
		return;
	}
	if(DBG) ALOGD("%s(%d)==================================================================", __func__, __LINE__);
	if(DBG) ALOGD("http request %s %s", method, uri);
	...
	challenge = http_request_get_header(request, "Apple-Challenge", index);
	if (!require_auth && challenge) {
		...
		if(DBG) ALOGD("Got challenge: %s", challenge);
		if(DBG) ALOGD("Got response: %s", signature);
	}
	//...
	if (!strcmp(method,"POST") && !strcmp(uri,"/pair-setup")) {
		...
		res = request_handle_setparameter(conn, request, res, &responseData, &responseDataLen, index);
	...
	http_response_add_header(res, "Server", "AirTunes/373.8");
	if(cseq) http_response_add_header(res, "CSeq", cseq);
	...
	if(DBG) ALOGD("Handled request %s with URL %s", method, uri);
	*response = res;
}
...
raop_t * raop_init(int max_clients, raop_callbacks_t *callbacks, const char *pemkey, int *error) {
	...
	raop->httpd = httpd_init(&httpd_cbs, max_clients, -1);
...
int raop_start(raop_t *raop, unsigned short *port, const char *hwaddr, int hwaddrlen, const char *password, int width, int height, int maxfps ) {
	assert(raop);
	assert(port);
	assert(hwaddr);
