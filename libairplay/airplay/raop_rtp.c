...
raop_rtp_parse_remote(raop_rtp_t *raop_rtp, const char *remote)
{
	...
	ret = netutils_parse_address(family, current, &raop_rtp->remote_saddr, sizeof(raop_rtp->remote_saddr));
	if (ret < 0) {
		free(original);
		return -1;
	}
	...
	struct sockaddr_in* sin = (struct sockaddr_in*)&raop_rtp->remote_saddr;
	raop_rtp->remote = (uint8_t*)&sin->sin_addr.s_addr;

	return 0;
}

raop_rtp_t *
raop_rtp_init(raop_callbacks_t *callbacks, const char *remote,
              ...
              const unsigned char *aeskey, const unsigned char *aesiv)
{
	...
	if (strstr(rtpmap, "AppleLossless")) {
		cn = 1;
	}

	if (!fmtp || strstr(fmtp, "AAC-eld")) {
		cn = 3;
	}
...
raop_rtp_resend_callback(void *opaque, unsigned short seqnum, unsigned short count)
{
	...
	if(DBG) ALOGD("Got resend request %d %d", seqnum, count);
	...
	if (ret == -1) {
		if(DBG) ALOGW("Resend failed: %d", SOCKET_GET_ERROR());
	}
...
raop_rtp_process_events(raop_rtp_t *raop_rtp)
{
	...
	if (volume_changed) {
		if (raop_rtp->callbacks.audio_set_volume) {
			raop_rtp->callbacks.audio_set_volume(raop_rtp->callbacks.cls, ceil(volume), raop_rtp->remote);
		}
	}
	...
	if (flush != NO_FLUSH) {
		...
		if (raop_rtp->callbacks.audio_flush) {
			raop_rtp->callbacks.audio_flush(raop_rtp->callbacks.cls, raop_rtp->remote);
		}
	}
...
raop_rtp_thread_udp(void *arg)
{
	...
	raop_rtp->callbacks.audio_init(raop_rtp->callbacks.cls,
	                               config->bitDepth,
	                               config->numChannels,
	                               config->sampleRate,
								   0, raop_rtp->remote);
	...
	while (1) {
		...
		if (raop_rtp_process_events(raop_rtp)) {
			break;
		}
		...
		if (FD_ISSET(raop_rtp->csock, &rfds)) {
			...
			packetlen = recvfrom(raop_rtp->csock, (char *)packet, sizeof(packet), 0, (struct sockaddr *)&saddr, &saddrlen);
			...
			if (packetlen >= 12) {
				...
				if(DBG) ALOGD("Got control packet of type 0x%02x", type);
				if (type == 0x56) {
					...
					*(int32_t *)&g_x56_buf[4] += 0x7C558180;
			...
			if(DBG) ALOGI("Would have timing packet in queue");
		} else if (FD_ISSET(raop_rtp->dsock, &rfds)) {
			saddrlen = sizeof(saddr);
			packetlen = recvfrom(raop_rtp->dsock, (char *)packet, sizeof(packet), 0, (struct sockaddr *)&saddr, &saddrlen);
			if (packetlen >= 12) {
				int no_resend = (raop_rtp->control_rport == 0);
				int ret;

				const void *audiobuf = 0;
				int audiobuflen = 0;
				...
				while ((audiobuf = raop_buffer_dequeue(raop_rtp->buffer, &audiobuflen, no_resend, &timestamp))) {
                    if (raop_rtp->callbacks.audio_process) {
                        raop_rtp->callbacks.audio_process(raop_rtp->callbacks.cls, audiobuf, audiobuflen, timestamp, 1, raop_rtp->remote);
                    }
				}
	...
	if(DBG) ALOGI("Exiting UDP RAOP thread");
	if (raop_rtp->callbacks.audio_destroy) {
        raop_rtp->callbacks.audio_destroy(raop_rtp->callbacks.cls, raop_rtp->remote);
    }
	return 0;
}
...
raop_rtp_thread_tcp(void *arg)
{
	...
	while (1) {
		...
		if (stream_fd == -1 && FD_ISSET(raop_rtp->dsock, &rfds)) {
			...
			if (stream_fd == -1) {
				/* FIXME: Error happened */
				if(DBG) ALOGI("Error in accept %d %s", errno, strerror(errno));
				break;
			}
		}
		if (stream_fd != -1 && FD_ISSET(stream_fd, &rfds)) {
			...
			if (rtplen > sizeof(packet)) {
				...
				if(DBG) ALOGI("Error, packet too long %d", rtplen);
				break;
			}
			...
			if ((audiobuf = raop_buffer_dequeue(raop_rtp->buffer, &audiobuflen, 1, &timestamp))) {
				if(DBG) ALOGI("audio_process11 :: %s",bin2hex((const unsigned char *)audiobuf,audiobuflen));
				raop_rtp->callbacks.audio_process(raop_rtp->callbacks.cls, audiobuf, audiobuflen, timestamp, 1, raop_rtp->remote);
			}
		}
	}
