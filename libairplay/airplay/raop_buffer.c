...
// 		audio_buffer_size = 1408
	...
	if (cn == 1) {
		...
		raop_buffer->alac = alac_create(alacConfig->bitDepth, alacConfig->numChannels);
		if (!raop_buffer->alac) {
	...
	if (et != 1 && et != 3) {
		/* Todo */
		fprintf(stderr, "Unknown encryption method! et=%d\n", et);
		return NULL;
	}
...
raop_buffer_queue(raop_buffer_t *raop_buffer, unsigned char *data, unsigned short datalen, int use_seqnum)
{
	...
	entry->type = data[1];
	entry->seqnum = seqnum;
	entry->timestamp = (data[4] << 24) | (data[5] << 16) | (data[6] << 8) | data[7];
	entry->ssrc = (data[8] << 24) | (data[9] << 16) | (data[10] << 8) | data[11];
	entry->available = 1;
	entry->resending = 0;
	...
	if (encryptedlen > 0)
	{
		...
		{
			memcpy(packetbuf, &data[12], encryptedlen);
		}
	}
...
const void *
raop_buffer_dequeue(raop_buffer_t *raop_buffer, int *length, int no_resend, unsigned int* timestamp)
{
	short buflen;
	raop_buffer_entry_t *entry;
